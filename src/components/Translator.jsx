...
export default function Translator() {
  ...
  const PLACEHOLDER = isLatinToJawa ? 'Ketik teks Latin di sini...' : 'Ketik aksara Jawa (gunakan keyboard di bawah)...';
  ...
  return (
    <div className="w-full max-w-5xl mx-auto px-4 sm:px-6 lg:px-8">
          ...
          >
            ...
            <div className="p-5 sm:p-6 flex-1 flex flex-col">
              ...
              <textarea
                ...
                className={`w-full flex-1 min-h-[160px] text-base sm:text-lg leading-relaxed bg-transparent text-zinc-900 dark:text-zinc-100 placeholder-zinc-400 dark:placeholder-zinc-600 resize-none focus:outline-none transition-all duration-300 ${!isLatinToJawa ? 'font-javanese text-2xl sm:text-3xl' : ''}`}
                spellCheck={false}
                autoComplete="off"
              />
      ...
      {/* ═══════════════ AKSARA TABLE ═════════════════════════════ */}
      <section className="mt-8 md:mt-12 animate-fade-in-delay-3">
        <button
          onClick={() => setShowTable(!showTable)}
          className="w-full flex items-center justify-between p-5 sm:p-6 rounded-3xl
            bg-white/80 dark:bg-white/[0.02] backdrop-blur-md
            border border-zinc-200/50 dark:border-white/10
            shadow-sm hover:shadow-md hover:border-amber-400/30 dark:hover:border-amber-500/30
            transition-all duration-300 cursor-pointer group text-left"
        >
          <div className="flex items-center gap-4">
            <div className="h-10 w-10 rounded-full bg-amber-50 dark:bg-amber-500/10 flex items-center justify-center text-amber-500 dark:text-amber-400 group-hover:scale-110 transition-transform duration-300">
              <LibraryIcon className="h-5 w-5" />
            </div>
            <div>
              <h3 className="text-base font-bold text-zinc-800 dark:text-zinc-100 group-hover:text-amber-600 dark:group-hover:text-amber-400 transition-colors">
                Tabel Aksara Dasar
              </h3>
              <p className="text-xs text-zinc-500 dark:text-zinc-400 mt-0.5">
                Klik karakter untuk menyalin (Carakan)
              </p>
            </div>
          </div>
          <div className={`h-8 w-8 rounded-full bg-zinc-100 dark:bg-white/5 flex items-center justify-center transition-all duration-500 ${showTable ? 'rotate-180 bg-amber-50 dark:bg-amber-500/20 text-amber-500' : 'text-zinc-400'}`}>
            <svg
        ...
        <div className={`overflow-hidden transition-all duration-500 ease-in-out
          ${showTable ? 'max-h-[800px] opacity-100 mt-4' : 'max-h-0 opacity-0 mt-0'}`}>
          <div className="grid grid-cols-3 sm:grid-cols-5 lg:grid-cols-10 gap-3 sm:gap-4 p-1">
            {AKSARA_TABLE.map(([latin, jawa]) => (
              <button
                key={latin}
                onClick={() => {
                  navigator.clipboard.writeText(jawa);
                  // Optional: add a tiny visual feedback here if needed
                }}
                title={`Salin ${latin} (${jawa})`}
                className="flex flex-col items-center justify-center p-4 sm:p-5 rounded-2xl
                  ...
                  hover:scale-[1.05] hover:shadow-lg hover:shadow-amber-500/10
                  active:scale-95
                  transition-all duration-300 group/cell cursor-pointer"
              >
                <span className="font-javanese text-2xl sm:text-3xl
                  text-amber-600 dark:text-amber-400
                  group-hover/cell:text-amber-500 dark:group-hover/cell:text-amber-300
                  transition-colors drop-shadow-sm group-hover/cell:drop-shadow-md">
                  {jawa}
                </span>
                <span className="text-[11px] sm:text-xs text-zinc-400 dark:text-zinc-500 mt-2 font-semibold uppercase tracking-wider">
                  {latin}
                </span>
              </button>
            ))}
      ...
      {/* ══════════════════ FOOTER ════════════════════════════════ */}
      <footer className="mt-16 md:mt-24 mb-8 pt-8 border-t border-zinc-200/50 dark:border-white/5 animate-fade-in-delay-3">
